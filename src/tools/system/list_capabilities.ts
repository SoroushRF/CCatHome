...
export const listCapabilitiesDefinition: CapabilityDefinition = {
  ...
  description:
    "Lists all available Tier B capabilities in the system, matching optional query filters. Never returns Tier A tools.",
  inputSchema: z.object({
    query: z
      .string()
      .optional()
      .describe("Optional query string to search capability names and descriptions"),
  }),
  tier: PermissionTier.TIER_0, // Tier 0: read-only discovery
};

export async function listCapabilitiesHandler(args: { query?: string }): Promise<{
  success: boolean;
